...
// Common currency issues from your data
const CURRENCY_NORMALIZATIONS = {
    '': 'USD',  // Empty string defaults to USD
    'null': 'USD',
    'undefined': 'USD',
    'NONE': 'USD',
    'N/A': 'USD'
...
function isDataQualityBetter(newValue, existingValue, fieldName, localLogger) {
    // Never replace existing value with null
    if (newValue === null || newValue === undefined) {
        localLogger.debug(`Skipping ${fieldName}: new value is null/undefined, keeping existing: ${existingValue}`);
        return false;
    }
    
    // If no existing value, any valid new value is better
    if (existingValue === null || existingValue === undefined) {
        localLogger.debug(`${fieldName}: no existing value, accepting new value: ${newValue}`);
        return true;
    }
    ...
    if (fieldName === 'description') {
        ...
        if (newLen < MIN_DESCRIPTION_LENGTH && existingLen >= MIN_DESCRIPTION_LENGTH) {
            localLogger.debug(`${fieldName}: new description too short (${newLen} chars) vs existing (${existingLen} chars)`);
            return false;
        }
    ...
    if (fieldName === 'headquarters' || fieldName === 'website') {
        ...
        if (newStr.length <= existingStr.length) {
            localLogger.debug(`${fieldName}: new value not longer than existing, keeping current`);
            return false;
        }
        return true;
    }
    ...
    if (typeof newValue === 'number' && typeof existingValue === 'number') {
        if (existingValue !== 0) {
            ...
            if (changeRatio > FINANCIAL_VALUE_CHANGE_THRESHOLD) {
                localLogger.warn(`${fieldName}: suspicious change detected: ${existingValue} -> ${newValue} (${(changeRatio * 100).toFixed(0)}% change), keeping existing value`);
                return false; // Too big a change, likely an error
            }
        }
        localLogger.debug(`${fieldName}: accepting new value ${newValue} (was ${existingValue})`);
    }
...
async function retryOperation(fn, operationName, ticker, retries = RETRY_COUNT, baseDelay = RETRY_DELAY_MS) {
    ...
    for (let i = 0; i <= retries; i++) {
        ...
        try {
            localLogger.debug(`Attempting ${operationName}, try ${i + 1}/${retries + 1}...`);
            const result = await fn();
            localLogger.debug(`${operationName} succeeded on try ${i + 1}`);
            return result;
        } catch (e) {
            ...
            if (i === retries || isHttpClientError || isQuoteNotFound) {
                ...
                localLogger.error(`${operationName} failed permanently (${reason}): ${errorMessage}`, { 
            ...
            localLogger.warn(`${operationName} error: ${errorMessage}. Retry ${i + 1}/${retries} in ${Math.round(delayMs)}ms...`);
            await delay(delayMs);
        }
    }
    ...
    try {
        ...
        rates.forEach(row => {
            if (row.from_currency && row.to_currency && isValidNumber(row.rate)) {
                ...
                // Track all known currencies
                supportedCurrencies.add(from);
                supportedCurrencies.add(to);
            }
        });
        
        logger.info(`Loaded ${rates.length} exchange rates into cache. Supported currencies: ${Array.from(supportedCurrencies).join(', ')}`);
        ...
        if (fallbacksAdded > 0) {
            logger.warn(`Added ${fallbacksAdded} fallback exchange rates. Consider updating your exchange_rates table.`);
        }
        
    } catch (err) {
        logger.error(`Failed to load exchange rates from Supabase: ${err.message}`, { stack: err.stack });
        logger.warn('Using comprehensive fallback exchange rates. Financial conversions may be inaccurate!');
        exchangeRatesCache = {
            'CAD': { 'USD': 0.73, 'EUR': 0.68, 'GBP': 0.58 },
            'AUD': { 'USD': 0.66, 'EUR': 0.61, 'GBP': 0.52 },
            'USD': { 'CAD': 1.37, 'AUD': 1.52, 'EUR': 0.93, 'GBP': 0.79 },
...
function convertToUSD(value, currency, operationContext = 'Conversion', localLogger = logger) {
    ...
    if (rate === null) {
        localLogger.error(`${operationContext}: Cannot convert ${normalizedCurrency} to USD - Exchange rate not found. Value: ${numericValue}. Available from currencies: ${Object.keys(exchangeRatesCache).join(', ')}`);
        return null;
    }
    ...
    try {
        ...
        try {
            ...
            if (quoteSummaryError.message?.includes('Schema validation') || 
                ...
                localLogger.warn(`Schema validation error for ${actualTicker}, attempting fallback approach: ${quoteSummaryError.message}`);
                ...
                for (const module of modules) {
                    try {
                        ...
                        localLogger.debug(`Successfully fetched module: ${module}`);
                    } catch (moduleError) {
        ...
        const receivedModules = Object.keys(result).filter(k => result[k] !== null);
        localLogger.info(`Received data for modules: ${receivedModules.join(', ')}`);
...
async function updateCompanyProfile(companyId, ticker, yahooSummaryProfile, existingCompanyData) {
    ...
    if (isDataQualityBetter(newDescription, existingCompanyData?.description, 'description', localLogger)) {
        ...
        localLogger.info(`Will update description (new: ${newDescription?.length || 0} chars, old: ${existingCompanyData?.description?.length || 0} chars)`);
    }
    ...
    if (isDataQualityBetter(newHeadquarters, existingCompanyData?.headquarters, 'headquarters', localLogger)) {
        ...
        localLogger.info(`Will update headquarters from "${existingCompanyData?.headquarters || 'null'}" to "${newHeadquarters}"`);
    }
    ...
    try {
        ...
        localLogger.error(`Error updating companies table: ${error.message}`, { 
...
async function updateCompanyWebsiteUrl(companyId, ticker, yahooSummaryProfile, existingCompanyUrls) {
    ...
    if (existingWebsiteEntry?.url === newWebsiteUrl) {
        localLogger.info(`Company website URL is already up-to-date: ${newWebsiteUrl}`);
    ...
    try {
        ...
        localLogger.warn(`Invalid URL format from Yahoo: "${newWebsiteUrl}" - ${e.message}`);
    ...
    try {
        ...
        localLogger.info(`Upserting website URL: "${existingWebsiteEntry?.url || 'none'}" -> "${newWebsiteUrl}"`);
        const { error } = await supabase.from('company_urls').upsert(upsertData, {
            onConflict: 'company_id, url_type',
        });
        
        if (error) {
            localLogger.error(`Error upserting company website URL: ${error.message}`, { 
                details: error.details,
                code: error.code,
                hint: error.hint 
            });
...
async function updateFinancialsTableInSupabase(companyId, ticker, yahooQuoteSummary, existingFinancials) {
    ...
    try {
        if (!yahooQuoteSummary?.price || !yahooQuoteSummary?.financialData || !yahooQuoteSummary?.defaultKeyStatistics) {
            localLogger.warn("Incomplete Yahoo quoteSummary for financials. Missing modules: " + 
                [!yahooQuoteSummary?.price && 'price', 
                 !yahooQuoteSummary?.financialData && 'financialData',
                 !yahooQuoteSummary?.defaultKeyStatistics && 'defaultKeyStatistics'].filter(Boolean).join(', '));
        ...
        // Get currency with better handling
        const rawCurrency = price.currency || existingFinancials?.cash_currency || null;
        const currency = normalizeCurrency(rawCurrency, `financials for ${ticker}`, localLogger);
        const currentDateISO = new Date().toISOString();
        
        // Log the currency situation
        localLogger.info(`Currency handling: Raw="${rawCurrency}", Normalized="${currency}", Existing="${existingFinancials?.cash_currency || 'none'}"`);
            ...
            if (timestamp > 0 && timestamp < 3155760000) {
                try {
                    ...
                    if (parsedDate.getFullYear() >= 1970 && parsedDate.getFullYear() <= 2070) {
                        ...
                        localLogger.debug(`Parsed report date: ${reportDate} from timestamp ${timestamp}`);
                    } else {
                        localLogger.warn(`Parsed date ${parsedDate} outside reasonable range`);
                    }
                } catch (e) {
                    localLogger.warn(`Error parsing date from timestamp ${timestamp}: ${e.message}`);
                }
            }
        }
        ...
        if (isValidNumber(newFinancialData.cash_value) && isValidNumber(newFinancialData.debt_value)) {
            ...
            localLogger.debug(`Calculated net_financial_assets: ${newFinancialData.net_financial_assets} = ${newFinancialData.cash_value} - ${newFinancialData.debt_value}`);
        }
        ...
        localLogger.debug(`Starting field comparison. Existing currency: ${existingFinancials?.cash_currency}, New currency: ${currency}`);
        ...
        for (const [key, newValue] of Object.entries(newFinancialData)) {
            ...
            if (key.endsWith('_value') || ['cash', 'debt', 'revenue', 'ebitda', 'net_income'].includes(key.replace('_value', ''))) {
                const baseName = key.replace('_value', '');
                const valueKey = key.endsWith('_value') ? key : `${key}_value`;
                const currencyKey = `${baseName}_currency`;
                const newCurrency = newFinancialData[currencyKey] || currency;
                const existingCurrency = existingFinancials?.[currencyKey];
                
                localLogger.debug(`Comparing ${valueKey}: new=${newValue} ${newCurrency}, existing=${existingValue} ${existingCurrency}`);
                
                // If we have a new value
                if (newValue !== null && newValue !== undefined) {
                    // If no existing value, accept new
                    if (existingValue === null || existingValue === undefined) {
                        finalPayload[valueKey] = newValue;
                        if (newCurrency) finalPayload[currencyKey] = newCurrency;
                        fieldsUpdated.push(valueKey);
                        localLogger.info(`${valueKey}: Setting new value ${newValue} ${newCurrency} (was null)`);
                        if (['market_cap_value', 'revenue_value', 'cash_value', 'debt_value'].includes(valueKey)) {
                            significantUpdate = true;
                        }
                    }
                    // If currencies match, compare values directly
                    else if (newCurrency === existingCurrency && Math.abs(newValue - existingValue) > 0.01) {
                        finalPayload[valueKey] = newValue;
                        if (newCurrency) finalPayload[currencyKey] = newCurrency;
                        fieldsUpdated.push(valueKey);
                        localLogger.info(`${valueKey}: Updating ${existingValue} -> ${newValue} ${newCurrency}`);
                        if (['market_cap_value', 'revenue_value', 'cash_value', 'debt_value'].includes(valueKey)) {
                            significantUpdate = true;
                        }
                    }
                    // If currencies don't match, need to compare USD values
                        ...
                        const newUSD = convertToUSD(newValue, newCurrency, `${valueKey} comparison`, localLogger);
                        const existingUSD = convertToUSD(existingValue, existingCurrency, `${valueKey} existing comparison`, localLogger);
                        
                        if (newUSD !== null && existingUSD !== null) {
                            ...
                            if (changeRatio < FINANCIAL_VALUE_CHANGE_THRESHOLD && changeRatio > 0.01) {
                                ...
                                fieldsUpdated.push(valueKey);
                                localLogger.info(`${valueKey}: Currency conversion update: ${existingValue} ${existingCurrency} -> ${newValue} ${newCurrency} (${(changeRatio * 100).toFixed(1)}% change in USD)`);
                                if (changeRatio > 0.05) significantUpdate = true;
                            } else if (changeRatio >= FINANCIAL_VALUE_CHANGE_THRESHOLD) {
                                localLogger.warn(`${valueKey}: Skipping due to extreme change: ${existingValue} ${existingCurrency} (${existingUSD} USD) -> ${newValue} ${newCurrency} (${newUSD} USD) - ${(changeRatio * 100).toFixed(0)}% change`);
                            }
                        } else {
                            localLogger.warn(`${valueKey}: Could not convert to USD for comparison. NewUSD=${newUSD}, ExistingUSD=${existingUSD}`);
                        }
                    }
                }
                // If no new value but we have existing, preserve it
                else if (existingValue !== null && existingValue !== undefined) {
                    finalPayload[valueKey] = existingValue;
                    if (existingCurrency) finalPayload[currencyKey] = existingCurrency;
                    localLogger.debug(`${valueKey}: Preserving existing value ${existingValue} ${existingCurrency}`);
                }
            }
            // For non-value fields
                ...
                if (existingValue === null || existingValue === undefined || 
                    ...
                    localLogger.info(`${key}: Updating ${existingValue} -> ${newValue}`);
                }
            }
            // Preserve existing non-null values
            else if (existingValue !== null && existingValue !== undefined) {
                finalPayload[key] = existingValue;
                localLogger.debug(`${key}: Preserving existing value ${existingValue}`);
            }
        }
        ...
        if (fieldsUpdated.length === 0) {
            localLogger.info("No financial data changes detected. All values either unchanged or failed quality checks.");
            return { updated: false, reason: "No changes to existing data" };
        }
        
        localLogger.info(`Preparing to update ${fieldsUpdated.length} financial fields: ${fieldsUpdated.join(', ')}. Significant update: ${significantUpdate}`);
        ...
        if (error) {
            localLogger.error(`Error upserting financials: ${error.message}`, { 
                details: error.details,
                code: error.code,
                hint: error.hint 
            });
        ...
        localLogger.info(`Successfully updated financials. Total fields: ${fieldsUpdated.length}, Significant: ${significantUpdate}`);
...
async function updateCapitalStructureInSupabase(companyId, ticker, yahooQuoteSummary, existingCapitalStructure) {
    ...
    // Check if update is needed (allow for small rounding differences)
    if (existingShares && Math.abs(newShares - existingShares) < 1) {
        localLogger.info(`Shares outstanding unchanged: ${newShares.toLocaleString()}`);
    ...
    if (existingShares && existingShares > 0) {
        ...
        if (changeRatio > 2.0) { // More than 200% change is suspicious
            localLogger.warn(`Suspicious change in shares: ${existingShares.toLocaleString()} -> ${newShares.toLocaleString()} (${(changeRatio * 100).toFixed(0)}% change). Skipping update.`);
    ...
    // Preserve other existing fields - NEVER overwrite with null
    ...
    try {
        localLogger.info(`Updating capital structure: shares ${existingShares?.toLocaleString() || 'null'} -> ${newShares.toLocaleString()}`);
        const { error } = await supabase
            .from('capital_structure')
            .upsert(capitalStructurePayload, { onConflict: 'company_id' });
            
        if (error) {
            localLogger.error(`Error upserting capital_structure: ${error.message}`, { 
                details: error.details,
                code: error.code,
                hint: error.hint 
            });
        ...
        localLogger.info(`Successfully updated capital structure. New shares: ${newShares.toLocaleString()}`);
        return { updated: true };
    } catch (error) {
        return { updated: false, error: true, errorMessage: error.message };
    }
...
async function processCompany(company) {
    ...
    if (!tsx_code || tsx_code.trim() === '') {
        localLogger.warn(`Skipping company "${company_name}" due to missing ticker.`);
    ...
    try {
        ...
        if (!finError && finData) {
            ...
            localLogger.debug(`Found existing financials data. Last updated: ${finData.last_updated || 'unknown'}`);
        }
        ...
        if (!capError && capData) {
            ...
            localLogger.debug(`Found existing capital structure data. Shares: ${capData.existing_shares?.toLocaleString() || 'unknown'}`);
        }
        ...
        localLogger.warn(`Error fetching existing data: ${e.message}. Proceeding with update anyway.`);
    }
    ...
    if (!yahooQuoteSummary) {
        localLogger.error(`Failed to fetch Yahoo data after all retries.`);
    ...
    try {
        ...
        if (significantUpdate || profileResult.updated || urlResult.updated) {
            ...
            if (companyUpdateError) {
                ...
                localLogger.debug(`Updated company last_updated timestamp`);
            }
        }
        
    } catch (processError) {
        localLogger.error(`Unexpected error during processing: ${processError.message}`, { stack: processError.stack });
        errorOccurred = true;
    }
