    ...
    const retryingFetch = origFetchRetry(fetcher as any, {
        ...
        retryOn: async (_attempt: number, error: any, response: any) => {
            ...
            if (response && response.error) {
                ...
                // Do not retry on 404 for RPC if it's specifically 'Function not found'
                if (response.status === 404 && (supaError.code === 'PGRST200' /* Function not found */ || supaError.message.toLowerCase().includes('function not found'))) {
...
export const FilterProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    console.log('[FilterContext] currentUserTier updated:', currentUserTier, 'isSubscriptionLoading:', isSubscriptionLoading);
  }, [currentUserTier, isSubscriptionLoading]);
  ...
  useEffect(() => {
    ...
    const fetchFullRanges = async () => {
      ...
      try {
        ...
        if (!mounted) return;
        console.error("[FilterContext] Error fetching ranges (after retries):", err.message, err);
  ...
  const fetchPaginatedDataOnly = useCallback(async (
    ...
    if (loadingRanges || loadingFilteredSet || loadingPaginated || (isSubscriptionLoading && !currentUserTier) ) {
      console.log(`[FilterContext] Skipping Paginated Fetch (page ${pageToFetch}) due to active loading or missing tier.`);
      return;
    }
    setLoadingPaginated(true); setError(null);
    console.log(`[FilterContext] Fetching Page Data Only: Page ${pageToFetch}, Size: ${sizeToFetch}, Tier: ${tier}`);
    ...
    try {
      ...
      if (fetchedData.length > 0 && data[0]?.total_rows !== undefined && totalCount !== data[0].total_rows) {
        console.warn(`[FilterContext] Paginated fetch total_rows (${data[0].total_rows}) differs from stored (${totalCount}).`);
      }
      lastFetchedPageRef.current = pageToFetch;
    } catch (err: any) {
      console.error(`[FilterContext] Error fetching paginated data (page ${pageToFetch}) (after retries):`, err.message, err);
  ...
  }, [buildFiltersJson, totalCount, loadingRanges, loadingFilteredSet, loadingPaginated, isSubscriptionLoading, currentUserTier ]);
  ...
  const fetchFilteredSetAndFirstPage = useCallback(async (
    ...
    if (isSubscriptionLoading && !currentUserTier) {
        console.log('[FilterContext] Subscription loading or tier unavailable, delaying initial data fetch.');
    ...
    console.log(`[FilterContext] Fetching Full Filtered Set & Page 1. PageSize: ${sizeForFirstPage}, Tier: ${tier}`);
    const filtersJson = buildFiltersJson(currentFilters);
    let fetchedTotalCount = 0; let firstPageItems: Company[] = EMPTY_ARRAY; let allFilteredIds: number[] = EMPTY_ID_ARRAY;
    ...
    try {
      ...
      if (pageOneError) throw new Error(`Failed to fetch initial page: ${pageOneError.message || 'RPC error'}`);

      if (!pageOneData || pageOneData.length === 0) {
        console.log('[FilterContext] No data for initial page.');
      } else {
        const rpcTotalRows = pageOneData[0]?.total_rows;
        if (typeof rpcTotalRows !== 'number' || !isFinite(rpcTotalRows)) throw new Error("Invalid total_rows.");
      ...
      if (fetchedTotalCount > 0) {
        ...
        if (idError) throw new Error(`Failed to fetch filtered IDs: ${idError.message || 'RPC error'}`);
        allFilteredIds = (idData as { company_id: number }[] || []).map(r => r.company_id);
        if (allFilteredIds.length !== fetchedTotalCount) console.warn(`[FilterContext] Count Mismatch: paginated total (${fetchedTotalCount}) vs. ID count (${allFilteredIds.length}).`);
      } else allFilteredIds = EMPTY_ID_ARRAY;

      setTotalCount(fetchedTotalCount); setDisplayData(firstPageItems); setFilteredCompanyIds(allFilteredIds);
      lastFetchedPageRef.current = 1;
    } catch (err: any) {
      console.error('[FilterContext] Error during fetchFilteredSetAndFirstPage (after retries):', err.message, err);
  ...
  }, [buildFiltersJson, isSubscriptionLoading, currentUserTier]);

  useEffect(() => {
    if (loadingRanges || (isSubscriptionLoading && !currentUserTier)) {
      console.log(`[FilterContext] Effect 1 (Filter Change): Skipping. ranges=${loadingRanges}, subLoad=${isSubscriptionLoading}, tier=${!!currentUserTier}`);
      return;
    }
    console.log("[FilterContext] Effect 1 (Filter Change) Triggered. Tier:", currentUserTier);
  ...
  useEffect(() => {
    if (loadingRanges || loadingFilteredSet || (isSubscriptionLoading && !currentUserTier)) {
      console.log(`[FilterContext] Effect 2 (Page Change): Skipping. ranges=${loadingRanges}, setLoad=${loadingFilteredSet}, subLoad=${isSubscriptionLoading}, tier=${!!currentUserTier}`);
      return;
    }
    if (currentPage === lastFetchedPageRef.current && currentPage !== DEFAULT_PAGE) return;
    if (currentPage === DEFAULT_PAGE) {
      if (displayData.length === 0 && totalCount > 0 && !loading) {
        console.log(`[FilterContext] Effect 2: Re-triggering page 1 fetch. total=${totalCount}`);
        fetchFilteredSetAndFirstPage(filterSettings, sortState, pageSize, currentCurrency, currentUserTier);
      }
      return;
    }
    console.log(`[FilterContext] Effect 2 (Page Change) Triggered: Fetching page ${currentPage}.`);
    fetchPaginatedDataOnly(currentPage, pageSize, sortState, filterSettings, currentCurrency, currentUserTier);
  }, [currentPage, pageSize, sortState, filterSettings, currentCurrency, currentUserTier, loadingRanges, loadingFilteredSet, isSubscriptionLoading, fetchPaginatedDataOnly, fetchFilteredSetAndFirstPage, displayData, totalCount, loading]);
  ...
  const fetchCompaniesByIds = useCallback(async (ids: number[]): Promise<Company[]> => {
    ...
    console.log(`[FilterContext] Fetching data for ${ids.length} companies by ID from VIEW 'companies_detailed_view'.`);
    setError(null);

    try {
      // Query the VIEW 'companies_detailed_view' directly
      const { data, error: viewError } = await supabase
        .from('companies_detailed_view') // This is a VIEW, not an RPC
        .select('*') // Select all columns from the view
        .in('company_id', ids); // Filter by the provided company_ids

      if (viewError) {
        console.error('[FilterContext] Error fetching from companies_detailed_view:', viewError);
        throw viewError; // Propagate the error
      }

      if (data === null) {
         console.log("[FilterContext] No data returned from companies_detailed_view for IDs:", ids);
        return EMPTY_ARRAY;
      }
      
      // Assuming RpcResponseRow is compatible or convertRpcRowsToCompanies handles view data
      const convertedData = convertRpcRowsToCompanies(data as RpcResponseRow[]); 
      console.log(`[FilterContext] Successfully fetched ${convertedData.length} companies from view.`);
      return convertedData ?? EMPTY_ARRAY;

    } catch (err: any) {
      console.error('[FilterContext] Final catch for fetchCompaniesByIds from view:', err);
      setError(`Failed to load detailed company data from view: ${err.message || 'Unknown error'}`);
      return EMPTY_ARRAY;
    }
  ...
  const handleResetFilters = useCallback(() => {
    console.log("[FilterContext] Resetting filters.");
    setFilterSettings(DEFAULT_FILTER_SETTINGS);
    setSortState(DEFAULT_SORT_STATE);
    setExcludedCompanyIds(EMPTY_SET);
    setPageSizeState(DEFAULT_PAGE_SIZE);
    // setCurrentPage(DEFAULT_PAGE); // Will be handled by the effect watching filterSettings/sortState
  }, []);
  ...
  const handleSetSort = useCallback((newSortState: SortState) => {
    if (newSortState.key !== sortState.key || newSortState.direction !== sortState.direction) setSortState(newSortState);
  }, [sortState]);
  ...
  const handleSetPage = useCallback((page: number) => {
    ...
    else if (page > maxPage && currentPage === maxPage) console.log(`[FilterContext] Page ${page} beyond max ${maxPage}.`);
  }, [currentPage, effectiveTotalCount, pageSize]);

  const handleSetPageSize = useCallback((size: number) => {
    if (pageSizeOptions.includes(size) && size !== pageSize) setPageSizeState(size);
  }, [pageSize]);
  ...
  const value = useMemo<FilterContextType>(() => ({
    ...
    loadingPaginated, loadingRanges, loadingFilteredSet, loading, error, setCurrentCurrency: handleSetCurrentCurrency,
    ...
    loadingFilteredSet, loading, error, handleSetCurrentCurrency, handleSetDevelopmentStatus, handleSetMetricRange,
