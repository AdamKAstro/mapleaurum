// src/contexts/filter-context.tsx
 ...
 export const FilterProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchWithRetry = useCallback(
   (rpcName: string, params: any) => {
    return fetchRetry(supabase.rpc, {
     retries: 3,
  ...
  useEffect(() => {
   ...
   const fetchFullRanges = async () => {
    ...
    try {
     const { data, error: rpcError } = await fetchWithRetry('get_metrics_ranges', {});
     if (!mounted) return;
  ...
  const fetchPaginatedDataOnly = useCallback(async (
   ...
   try {
    const { data, error: rpcError } = await fetchWithRetry('get_companies_paginated', {
  ...
  }, [buildFiltersJson, totalCount, isSubscriptionLoading, loadingRanges, loadingFilteredSet, loadingPaginated, fetchWithRetry]); // Dependencies for the callback itself
  ...
  const fetchFilteredSetAndFirstPage = useCallback(async (
   ...
   try {
    ...
    const { data: pageOneData, error: pageOneError } = await fetchWithRetry('get_companies_paginated', {
     page_num: 1,
    ...
    if (fetchedTotalCount > 0) {
     ...
     const { data: idData, error: idError } = await fetchWithRetry('get_filtered_company_ids', {
      filters: filtersJson,
      // p_user_tier: tier // If your RPC needs the tier
     });
  ...
  }, [buildFiltersJson, isSubscriptionLoading, fetchWithRetry]);
