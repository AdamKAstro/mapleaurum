...
const DEBUG_METRIC_TYPES = process.env.NODE_ENV === 'development'; // For local debugging
...
export function getMetricsByCategory(category: MetricCategory): MetricConfig[] {
  ...
  if (!Array.isArray(metrics)) {
    if (DEBUG_METRIC_TYPES) console.error("[metric-types] 'metrics' is not an array in getMetricsByCategory.");
    return [];
  }
...
export function getMetricByKey(key: string): MetricConfig | undefined {
  if (!key || typeof key !== 'string') {
    if (DEBUG_METRIC_TYPES) console.warn("[metric-types] Invalid key provided to getMetricByKey:", key);
    return undefined;
  }
  return metricsByKey.get(key);
}

// Ensuring getMetricConfig is available as it's used in index.tsx (via alias)
...
export function getMetricByDbColumn(dbColumn: string): MetricConfig | undefined {
  if (!dbColumn || typeof dbColumn !== 'string') {
    if (DEBUG_METRIC_TYPES) console.warn("[metric-types] Invalid dbColumn provided to getMetricByDbColumn:", dbColumn);
    return undefined;
  }
...
export function getMetricByNestedPath(nestedPath: string): MetricConfig | undefined {
  if (!nestedPath || typeof nestedPath !== 'string') {
    if (DEBUG_METRIC_TYPES) console.warn("[metric-types] Invalid nestedPath provided to getMetricByNestedPath:", nestedPath);
    return undefined;
  }
...
export function getAccessibleMetrics(tier: ColumnTier | undefined | null): MetricConfig[] {
  ...
  if (DEBUG_METRIC_TYPES) console.log(`[metric-types.ts/getAccessibleMetrics] Called with effectiveTier: ${effectiveTier}`);

  if (accessibleMetricsCache.has(effectiveTier)) {
    if (DEBUG_METRIC_TYPES) console.log(`[metric-types.ts/getAccessibleMetrics] Cache hit for tier ${effectiveTier}. Returning ${accessibleMetricsCache.get(effectiveTier)!.length} metrics.`);
    return accessibleMetricsCache.get(effectiveTier)!;
  }

  if (!Array.isArray(metrics)) {
    if (DEBUG_METRIC_TYPES) console.error("[metric-types] 'metrics' array is not defined or not an array in getAccessibleMetrics.");
    return [];
  }
  ...
  if (DEBUG_METRIC_TYPES) console.log(`[metric-types.ts/getAccessibleMetrics] User tier level calculated as: ${userTierLevel} (for effectiveTier: ${effectiveTier})`);

  if (userTierLevel === undefined) {
    if (DEBUG_METRIC_TYPES) console.warn(`[metric-types] Unknown effectiveTier value provided: ${effectiveTier}. Defaulting to show only 'free' metrics.`);
    const freeMetrics = metrics.filter(metric => metric.tier === 'free');
    accessibleMetricsCache.set(effectiveTier, freeMetrics);
    if (DEBUG_METRIC_TYPES) console.log(`[metric-types.ts/getAccessibleMetrics] Returning ${freeMetrics.length} 'free' metrics due to unknown tier.`);
    return freeMetrics;
  }

  const accessibleMetricsResult = metrics.filter(metric => {
    const metricTierLevel = tierLevels[metric.tier];
    if (metricTierLevel === undefined) {
      if (DEBUG_METRIC_TYPES) console.warn(`[metric-types] Metric '${metric.key}' has an unknown tier: '${metric.tier}'. Assuming inaccessible.`);
      return false;
    }
    return metricTierLevel <= userTierLevel; 
  });
  
  if (DEBUG_METRIC_TYPES) console.log(`[metric-types.ts/getAccessibleMetrics] Filtering complete. Found ${accessibleMetricsResult.length} metrics for userTierLevel ${userTierLevel} (effectiveTier ${effectiveTier}).`);
  if (DEBUG_METRIC_TYPES && accessibleMetricsResult.length < 20 && effectiveTier === 'premium') { 
      console.warn(`[metric-types.ts/getAccessibleMetrics] POTENTIAL ISSUE: Premium tier only has ${accessibleMetricsResult.length} metrics. Check tier definitions in 'metrics' array.`);
  }

  accessibleMetricsCache.set(effectiveTier, accessibleMetricsResult);
  return accessibleMetricsResult;
}

export function clearMetricCaches() {
  metricsByCategoryCache.clear();
  accessibleMetricsCache.clear();
  if (DEBUG_METRIC_TYPES) console.log("[metric-types.ts] All metric caches cleared.");
}