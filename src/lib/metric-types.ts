...
import type { ColumnTier, MetricCategory, MetricFormat } from './types'; // Ensure MetricCategory is imported
...
export interface MetricConfig {
  ...
  db_column: string;       // Flat column name from RpcResponseRow (e.g., 'f_market_cap_value') - used for filter ranges & backend sort
  nested_path: string;     // Dot-notation path in the nested Company object (e.g., 'financials.market_cap_value') - used for frontend data access
  unit: string;            // e.g., "USD", "$M", "%", "Moz", "x", "yrs", "$/oz"
  higherIsBetter: boolean; // true = Higher is better (↑), false = Lower is better (↓)
  category: MetricCategory; // Grouping for UI (e.g., "financials", "valuation-metrics")
  tier: ColumnTier;        // 'free' | 'pro' | 'premium'
  format: MetricFormat;    // How to display the number/value
  description: string;     // Detailed description for tooltips/help (user-provided)
}
...
// Descriptions are taken from user's HelpMetricsPage.tsx content.
// Tiers for new metrics are assigned based on user input.
// Existing tiers 'medium' are updated to 'pro'.

export const metrics: MetricConfig[] = [
  // === Company Overview - Free Tier ===
  {
    key: 'company-overview.percent_gold', // Unique key for React components
    db_column: 'percent_gold',            // Column name in RpcResponseRow & for backend filtering/sorting
    nested_path: 'percent_gold',          // Path to access data in the nested Company object
    label: 'Gold %',
  ...
  { // Existing metric - Verified
    ...
    unit: 'M', higherIsBetter: false, category: 'financials', tier: 'free', format: 'number' // Assuming free based on original structure
  },

  // === Financials - Pro Tier ===
  { // Existing metric - Tier was 'medium', now 'pro'. Description from user.
    ...
    unit: '$M', higherIsBetter: false, category: 'financials', tier: 'pro', format: 'currency'
  },
  { // Existing metric - Tier was 'medium', now 'pro'. Description from user.
    ...
    unit: '$M', higherIsBetter: true, category: 'financials', tier: 'pro', format: 'currency'
  },
  { // Existing metric - Tier was 'medium', now 'pro'. Description from user.
  ...
  { // Existing metric - Verified. Tier: pro. Description from user.
    ...
    unit: 'x', higherIsBetter: false, category: 'financials', tier: 'pro', format: 'ratio'
  },
  { // Existing metric - Verified. Tier: pro. Description from user.
  ...
  // NEW Financial Metrics - Pro Tier (User Input)
  {
    key: 'financials.peg_ratio', db_column: 'f_peg_ratio', nested_path: 'financials.peg_ratio',
    label: 'PEG Ratio', unit: 'x', higherIsBetter: false, category: 'financials', tier: 'pro', format: 'ratio',
  ...
  {
    key: 'financials.cost_of_revenue', db_column: 'f_cost_of_revenue', nested_path: 'financials.cost_of_revenue',
    label: 'Cost of Revenue', unit: '$M', higherIsBetter: false, category: 'financials', tier: 'pro', format: 'currency',
  ...
  {
    key: 'financials.gross_profit', db_column: 'f_gross_profit', nested_path: 'financials.gross_profit',
    label: 'Gross Profit', unit: '$M', higherIsBetter: true, category: 'financials', tier: 'pro', format: 'currency',
  ...
  {
    key: 'financials.operating_expense', db_column: 'f_operating_expense', nested_path: 'financials.operating_expense',
    label: 'Operating Expense', unit: '$M', higherIsBetter: false, category: 'financials', tier: 'pro', format: 'currency',
  ...
  {
    key: 'financials.operating_income', db_column: 'f_operating_income', nested_path: 'financials.operating_income',
    label: 'Operating Income', unit: '$M', higherIsBetter: true, category: 'financials', tier: 'pro', format: 'currency',
  ...
  {
    key: 'financials.liabilities', db_column: 'f_liabilities', nested_path: 'financials.liabilities',
    label: 'Total Liabilities', unit: '$M', higherIsBetter: false, category: 'financials', tier: 'pro', format: 'currency',
  ...
  // Text fields f_investments_json, f_hedgebook - not added as "metrics" unless a specific display requirement comes up.

  // === Financials - Premium Tier ===
  { // Existing metric - Verified. Tier: premium. Description from user.
  ...
  { // Existing metric - Verified. Tier: pro. Description from user.
    ...
    unit: 'M', higherIsBetter: false, category: 'capital-structure', tier: 'pro', format: 'number'
  },
  { // Existing metric - Verified. Tier: pro. Description from user.
  ...
  { // Existing metric - Tier was 'medium', now 'pro'. Description from user.
    ...
    unit: 'Moz', higherIsBetter: true, category: 'mineral-estimates', tier: 'pro', format: 'moz'
  },
  { // Existing metric - Tier was 'medium', now 'pro'. Description from user.
  ...
  { // Existing metric - Tier: pro. Description from user.
  ...
  // NEW Mineral Estimates - Pro Tier (User Input)
  {
    key: 'mineral_estimates.reserves_non_precious_aueq_moz', db_column: 'me_reserves_non_precious_aueq_moz', nested_path: 'mineral_estimates.reserves_non_precious_aueq_moz',
    label: 'Non-Precious Reserves', unit: 'Moz', higherIsBetter: true, category: 'mineral-estimates', tier: 'pro', format: 'moz',
  ...
  {
    key: 'mineral_estimates.measured_indicated_non_precious_aueq_moz', db_column: 'me_measured_indicated_non_precious_aueq_moz', nested_path: 'mineral_estimates.measured_indicated_non_precious_aueq_moz',
    label: 'Non-Precious M&I', unit: 'Moz', higherIsBetter: true, category: 'mineral-estimates', tier: 'pro', format: 'moz',
  ...
  {
    key: 'mineral_estimates.resources_non_precious_aueq_moz', db_column: 'me_resources_non_precious_aueq_moz', nested_path: 'mineral_estimates.resources_non_precious_aueq_moz',
    label: 'Non-Precious Resources', unit: 'Moz', higherIsBetter: true, category: 'mineral-estimates', tier: 'pro', format: 'moz',
  ...
  {
    key: 'mineral_estimates.mineable_total_aueq_moz', db_column: 'me_mineable_total_aueq_moz', nested_path: 'mineral_estimates.mineable_total_aueq_moz',
    label: 'Mineable Total', unit: 'Moz', higherIsBetter: true, category: 'mineral-estimates', tier: 'pro', format: 'moz',
  ...
  {
    key: 'mineral_estimates.mineable_precious_aueq_moz', db_column: 'me_mineable_precious_aueq_moz', nested_path: 'mineral_estimates.mineable_precious_aueq_moz',
    label: 'Mineable Precious', unit: 'Moz', higherIsBetter: true, category: 'mineral-estimates', tier: 'pro', format: 'moz',
  ...
  {
    key: 'mineral_estimates.mineable_non_precious_aueq_moz', db_column: 'me_mineable_non_precious_aueq_moz', nested_path: 'mineral_estimates.mineable_non_precious_aueq_moz',
    label: 'Mineable Non-Precious', unit: 'Moz', higherIsBetter: true, category: 'mineral-estimates', tier: 'pro', format: 'moz',
  ...
  { // Existing metric - Tier: premium. Description from user.
  ...
  // NEW Mineral Estimates - Premium Tier (User Input)
  {
    key: 'mineral_estimates.potential_non_precious_aueq_moz', db_column: 'me_potential_non_precious_aueq_moz', nested_path: 'mineral_estimates.potential_non_precious_aueq_moz',
    label: 'Non-Precious Potential', unit: 'Moz', higherIsBetter: true, category: 'mineral-estimates', tier: 'premium', format: 'moz',
  ...
  { // Existing metric - Tier was 'medium', now 'pro'. Description from user.
    ...
    unit: '$/oz', higherIsBetter: false, category: 'valuation-metrics', tier: 'pro', format: 'currency'
  },
  { // Existing metric - Tier was 'medium', now 'pro'. Description from user.
  ...
  // NEW Valuation Metrics - Pro Tier (User Input)
  {
    key: 'valuation_metrics.mkt_cap_per_mi_oz_all', db_column: 'vm_mkt_cap_per_mi_oz_all', nested_path: 'valuation_metrics.mkt_cap_per_mi_oz_all',
    label: 'MC/M&I oz', unit: '$/oz', higherIsBetter: false, category: 'valuation-metrics', tier: 'pro', format: 'currency',
  ...
  {
    key: 'valuation_metrics.mkt_cap_per_mi_oz_precious', db_column: 'vm_mkt_cap_per_mi_oz_precious', nested_path: 'valuation_metrics.mkt_cap_per_mi_oz_precious',
    label: 'MC/Prec M&I oz', unit: '$/oz', higherIsBetter: false, category: 'valuation-metrics', tier: 'pro', format: 'currency',
  ...
  {
    key: 'valuation_metrics.mkt_cap_per_mineable_oz_all', db_column: 'vm_mkt_cap_per_mineable_oz_all', nested_path: 'valuation_metrics.mkt_cap_per_mineable_oz_all',
    label: 'MC/Mineable oz', unit: '$/oz', higherIsBetter: false, category: 'valuation-metrics', tier: 'pro', format: 'currency',
  ...
  {
    key: 'valuation_metrics.mkt_cap_per_mineable_oz_precious', db_column: 'vm_mkt_cap_per_mineable_oz_precious', nested_path: 'valuation_metrics.mkt_cap_per_mineable_oz_precious',
    label: 'MC/Prec Mineable oz', unit: '$/oz', higherIsBetter: false, category: 'valuation-metrics', tier: 'pro', format: 'currency',
  ...
  {
    key: 'valuation_metrics.mkt_cap_per_production_oz', db_column: 'vm_mkt_cap_per_production_oz', nested_path: 'valuation_metrics.mkt_cap_per_production_oz',
    label: 'MC/Production oz', unit: '$/oz', higherIsBetter: false, category: 'valuation-metrics', tier: 'pro', format: 'currency',
  ...
  // NEW Valuation Metrics - Premium Tier (User Input)
  {
    key: 'valuation_metrics.ev_per_mi_oz_all', db_column: 'vm_ev_per_mi_oz_all', nested_path: 'valuation_metrics.ev_per_mi_oz_all',
    label: 'EV/M&I oz', unit: '$/oz', higherIsBetter: false, category: 'valuation-metrics', tier: 'premium', format: 'currency',
  ...
  {
    key: 'valuation_metrics.ev_per_mi_oz_precious', db_column: 'vm_ev_per_mi_oz_precious', nested_path: 'valuation_metrics.ev_per_mi_oz_precious',
    label: 'EV/Prec M&I oz', unit: '$/oz', higherIsBetter: false, category: 'valuation-metrics', tier: 'premium', format: 'currency',
  ...
  {
    key: 'valuation_metrics.ev_per_mineable_oz_all', db_column: 'vm_ev_per_mineable_oz_all', nested_path: 'valuation_metrics.ev_per_mineable_oz_all',
    label: 'EV/Mineable oz', unit: '$/oz', higherIsBetter: false, category: 'valuation-metrics', tier: 'premium', format: 'currency',
  ...
  {
    key: 'valuation_metrics.ev_per_mineable_oz_precious', db_column: 'vm_ev_per_mineable_oz_precious', nested_path: 'valuation_metrics.ev_per_mineable_oz_precious',
    label: 'EV/Prec Mineable oz', unit: '$/oz', higherIsBetter: false, category: 'valuation-metrics', tier: 'premium', format: 'currency',
  ...
  {
    key: 'valuation_metrics.ev_per_production_oz', db_column: 'vm_ev_per_production_oz', nested_path: 'valuation_metrics.ev_per_production_oz',
    label: 'EV/Production oz', unit: '$/oz', higherIsBetter: false, category: 'valuation-metrics', tier: 'premium', format: 'currency',
  ...
  { // Existing metric - Verified. Tier: free. Description from user.
  ...
  { // Existing metric - Verified. Tier: pro. Description from user.
    ...
    unit: 'years', higherIsBetter: true, category: 'production', tier: 'pro', format: 'years'
  },
  { // Existing metric - Verified. Tier: pro. Description from user.
    ...
    unit: 'koz', higherIsBetter: true, category: 'production', tier: 'pro', format: 'koz'
  },
  { // Existing metric - Verified. Tier: pro. Description from user.
  ...
  // NEW Cost Metrics - Premium Tier (User Input)
  {
    key: 'costs.aic_last_quarter', db_column: 'c_aic_last_quarter', nested_path: 'costs.aic_last_quarter',
    label: 'Last Qtr AIC', unit: '$/oz', higherIsBetter: false, category: 'costs', tier: 'premium', format: 'currency',
  ...
  {
    key: 'costs.aic_last_year', db_column: 'c_aic_last_year', nested_path: 'costs.aic_last_year',
    label: 'Last Yr AIC', unit: '$/oz', higherIsBetter: false, category: 'costs', tier: 'premium', format: 'currency',
  ...
  {
    key: 'costs.tco_current', db_column: 'c_tco_current', nested_path: 'costs.tco_current',
    label: 'Current TCO', unit: '$/oz', higherIsBetter: false, category: 'costs', tier: 'premium', format: 'currency',
...
// --- Helper Functions (ensure these remain functional) ---
export function getMetricsByCategory(category: MetricCategory): MetricConfig[] {
  if (!Array.isArray(metrics)) { // Defensive check
...
export function getMetricByKey(key: string): MetricConfig | undefined {
  if (!Array.isArray(metrics)) {
    console.error("[metric-types] 'metrics' is not an array in getMetricByKey.");
    return undefined;
  }
...
export function getMetricByDbColumn(dbColumn: string): MetricConfig | undefined {
  if (!Array.isArray(metrics)) {
    console.error("[metric-types] 'metrics' is not an array in getMetricByDbColumn.");
    return undefined;
  }
...
export function getMetricByNestedPath(nestedPath: string): MetricConfig | undefined {
  if (!Array.isArray(metrics)) {
    console.error("[metric-types] 'metrics' is not an array in getMetricByNestedPath.");
    return undefined;
  }
...
export function getAccessibleMetrics(tier: ColumnTier | undefined | null): MetricConfig[] {
  ...
  if (!tier) { // Handle undefined or null tier gracefully, perhaps defaulting to 'free' equivalent
    console.warn("[metric-types] Undefined or null tier provided to getAccessibleMetrics. Defaulting to 'free' access level.");
    tier = 'free';
  }
  ...
  if (userTierLevel === undefined) { // Should not happen if tier is validated above
    console.warn(`[metric-types] Unknown user tier value provided: ${tier}. Defaulting to show only 'free' metrics.`);
