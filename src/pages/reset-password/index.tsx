...
export function ResetPasswordPage() {
  ...
  useEffect(() => {
    const handleRecoveryToken = async () => {
      ...
      console.log('[ResetPasswordPage] URL tokens:', {
      ...
      // If we have recovery tokens, let Supabase handle them
      if (accessToken && type === 'recovery') {
        try {
          // Method 1: Let Supabase auto-detect the URL
          const { data: { session }, error } = await supabase.auth.getSession();

          if (!session && !error) {
            // Method 2: Manually set the session if auto-detect failed
            console.log('[ResetPasswordPage] Manually setting session from tokens');
            ...
            if (setError) {
              console.error('[ResetPasswordPage] Error setting session:', setError);
              setSessionError('Failed to validate reset link. Please request a new one.');
            } else if (data.session) {
              console.log('[ResetPasswordPage] Session set successfully');
              setHasValidSession(true);
              // Clear the hash from URL for security
              window.history.replaceState(null, '', window.location.pathname);
            }
          } else if (session) {
            console.log('[ResetPasswordPage] Session auto-detected');
            setHasValidSession(true);
            // Clear the hash from URL for security
            window.history.replaceState(null, '', window.location.pathname);
          }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-navy-900 via-navy-800 to-navy-900 p-4 text-white">
      ...
      <div className="w-full max-w-md text-center mb-8">
        <CardTitle className="text-3xl sm:text-4xl font-bold text-cyan-300">Reset Your Password</CardTitle>
              ...
              <>
                ...
                <Alert className="bg-blue-900/20 border-blue-600/30">
                  ...
                  <AlertDescription className="text-blue-300 text-xs">
                    <strong>Security tip:</strong> Use a unique password that you don't use for other accounts. Consider
            ...
            {(sessionError || (!hasValidSession && !isCheckingSession)) && !success && (
              <div className="text-center py-4 space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-gray-400">Password reset links expire after 1 hour for security reasons.</p>
                </div>
              </div>
            )}
          </form>
      ...
      <div className="mt-6 text-center text-sm text-gray-400 max-w-md space-y-2">
        ...
        {loading && (
          <p className="text-xs animate-pulse">Please do not close this window while we update your password...</p>
        )}
      </div>
    </div>
  );
