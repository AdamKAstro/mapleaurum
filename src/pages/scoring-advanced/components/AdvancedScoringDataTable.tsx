...
const RowTooltipContent: React.FC<{ result: AdvancedScoringResult }> = ({ result }) => {
    const topDrivers = useMemo(() => {
        return Object.entries(result.breakdown ?? {}) // Optional chaining for empty breakdown
            .filter(([, data]) => (data?.contribution ?? 0) > 0)
            .sort(([, a], [, b]) => (b?.contribution ?? 0) - (a?.contribution ?? 0))
            .slice(0, 3);
    }, [result.breakdown]);

    if (topDrivers.length === 0) {
        return <p className="text-xs">No significant positive score drivers available.</p>; // Slightly clearer message
    }
            ...
            <ul className="space-y-1 text-xs">
                {topDrivers.map(([key, data]) => {
                    ...
                    return <li key={key} className="flex justify-between gap-4">
                        ...
                        <span className="font-mono text-green-400">+{ (data?.contribution ?? 0).toFixed(2) }</span>
                    </li>
                })}
            </ul>
        </div>
                    ...
                    <TableBody>
                        {paginatedResults.map((result, index) => {
                            ...
                            <React.Fragment key={result.company.company_id}>
                                ...
                                <TableRow 
                                    onClick={() => setExpandedRow(expandedRow === absoluteIndex ? null : absoluteIndex)} 
                                    className="cursor-pointer hover:bg-navy-600/50"
                                    aria-label={`Expand row for ${result.company.company_name}`} // Accessibility
                                >
                                    ...
                                    <TableCell className="text-right">
                                        <div className="flex items-center justify-end gap-1 font-mono text-xs">
                                            <div className="w-12 h-1.5 bg-navy-800 rounded-full"><div className="h-1.5 bg-green-500 rounded-full" style={{ width: `${(result.confidenceScore ?? 0) * 100}%`}}></div></div>
                                            <span>{((result.confidenceScore ?? 0) * 100).toFixed(0)}%</span>
                                        </div>
                                    </TableCell>
                                    <TableCell className="text-center text-sm text-muted-foreground">{Object.keys(result.breakdown ?? {}).length}</TableCell> {/* Optional chaining */}
                                    <TableCell className="text-center">
                                        <ChevronDown 
                                            className={cn("h-4 w-4 transition-transform", expandedRow === absoluteIndex && "rotate-180")} 
                                            aria-label={expandedRow === absoluteIndex ? "Collapse" : "Expand"} // Accessibility
                                        />
                                    </TableCell>
                                </TableRow>
                                </TooltipTrigger>
                                            ...
                                            <TableCell colSpan={6}>
                                                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.2 }} className="p-4 overflow-hidden">
                                                    ...
                                                    <div className="text-xs space-y-1 max-h-60 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-navy-600">
                                                        {Object.entries(result.breakdown ?? {}).length > 0 ? ( // Fallback for empty
                                                            ...
                                                            <p className="text-center text-muted-foreground">No breakdown data available for this company.</p>
                                                        )}
                                                    </div>
                                                </motion.div>
                                            </TableCell>
