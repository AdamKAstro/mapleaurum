...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter } from '../../components/ui/dialog'; // For Modal
...
// Constants
const SHARE_PRICE_DB_COLUMN = 'share_price'; // Example, if you specifically exclude it
const EMPTY_COMPANY_ARRAY: Company[] = [];
const EMPTY_SCORE_ARRAY: CompanyScore[] = [];
const FILTERABLE_STATUSES: CompanyStatus[] = ['Producer', 'Developer', 'Explorer', 'Royalty', 'Other']; // Case as displayed in UI
...
const ScoringPage: React.FC = () => {
    ...
    const metricsForScoring = useMemo(() => {
        if (!currentUserTier) {
            console.warn("[ScoringPage] currentUserTier is not yet available for metricsForScoring.");
            return [];
        }
        const accessible = allMetricsFromTypes.filter(metric =>
            metric.db_column !== SHARE_PRICE_DB_COLUMN && // Example exclusion
            isFeatureAccessible(metric.tier, currentUserTier)
        );
        console.log(`[ScoringPage] Tier: ${currentUserTier}. Found ${accessible.length} accessible metrics for scoring.`);
        return accessible;
    }, [currentUserTier]);

    // State for metric weights
    const [metricWeights, setMetricWeights] = useState<Record<string, number>>({});

    // Effect to initialize or update metric weights when accessible metrics change
    useEffect(() => {
        console.log("[ScoringPage] Updating metricWeights. Accessible metrics count:", metricsForScoring.length);
        setMetricWeights(prevWeights => {
            ...
            metricsForScoring.forEach(metric => {
                newWeights[metric.db_column] = prevWeights.hasOwnProperty(metric.db_column)
                    ? prevWeights[metric.db_column] // Preserve existing weight if metric still accessible
                    : 100; // Default to 100 if new or re-enabled
                if (newWeights[metric.db_column] !== prevWeights[metric.db_column]) {
                    hasChanges = true;
                }
            });
            // Check if any weights need to be removed (metrics no longer in metricsForScoring)
            if (Object.keys(prevWeights).some(dbCol => !newWeights.hasOwnProperty(dbCol)) ||
                Object.keys(newWeights).length !== Object.keys(prevWeights).length) {
                hasChanges = true;
            }

            if (hasChanges) {
                console.log("[ScoringPage] Metric weights updated/initialized:", newWeights);
                return newWeights;
            }
            return prevWeights; // No changes needed
        });
    }, [metricsForScoring]);

    // State for filtering ranked companies by status (UI only)
    ...
    const groupedMetricsForUI = useMemo(() => {
        ...
        // Filter out metrics not intended for direct weighting/scoring if any
        ...
        allPotentiallyScorableMetrics.forEach(metric => {
            if (!metric.category) return; // Should always have a category if well-defined
    ...
    useEffect(() => {
        ...
        const fetchDataForScoring = async () => {
            if (loadingFilteredSet) {
                ...
                console.log("[ScoringPage][DataFetch] Waiting: filteredCompanyIds from context are still loading.");
                return;
            }
            ...
            if (idsToFetch.length === 0) {
                if (isMounted) {
                    console.log("[ScoringPage][DataFetch] No filteredCompanyIds available to fetch. Clearing scoring data.");
            ...
            setIsScoringDataLoading(true);
            setScoringCompanyData(EMPTY_COMPANY_ARRAY); // Clear previous data
            console.log(`[ScoringPage][DataFetch] Active: Fetching details for ${idsToFetch.length} unique companies. Sample IDs: ${idsToFetch.slice(0, 5).join(', ')}`);
            ...
            try {
                ...
                if (isMounted) {
                    ...
                    console.log(`[ScoringPage][DataFetch] Success: Loaded details for ${fetchedData.length} companies.`);
                    // --- B2Gold specific logging (replace 25 with actual ID or use name) ---
                    ...
                    if (b2GoldData) {
                        console.log(`[ScoringPage][DataFetch][B2GoldCheck] B2Gold (or similar name match, ID: ${b2GoldData.company_id}) IS PRESENT in scoringCompanyData. Status: ${b2GoldData.status}`);
                    } else {
                        console.warn(`[ScoringPage][DataFetch][B2GoldCheck] B2Gold (or similar name match, e.g., ID ${B2GOLD_COMPANY_ID_EXAMPLE}) NOT FOUND in scoringCompanyData after fetch.`);
                    }
                }
            } catch (e: any) {
                console.error('[ScoringPage][DataFetch] Error fetching company details for scoring:', e.message, e);
    ...
    // Callbacks for metric weight adjustments
    ...
    // Memoized calculation of company scores
    const calculatedScores: CompanyScore[] = useMemo(() => {
        console.log("[ScoringPage][CalcMemo] Recalculating scores. Triggered by dependency change.");
        ...
        console.log(`[ScoringPage][CalcMemo] Scoring with ${includedScoringData.length} companies after exclusions.`);
        ...
        if (isScoringDataLoading || loadingRanges || includedScoringData.length === 0 ||
            ...
            !currentUserTier) {
            console.warn("[ScoringPage][CalcMemo] Pre-conditions for scoring not met. Returning empty scores. Loadings:",
                `scoringData=${isScoringDataLoading}, ranges=${loadingRanges}. Data availability:`,
        ...
        metricsForScoring.forEach(metric => {
            if (metricWeights.hasOwnProperty(metric.db_column) &&
                isValidNumber(metricWeights[metric.db_column]) && // Uses your util
        ...
        if (Object.keys(activeAndAccessibleWeights).length === 0) {
            console.log("[ScoringPage][CalcMemo] No active and accessible weights with value > 0. Returning empty scores.");
            return EMPTY_SCORE_ARRAY;
        }

        console.log(`[ScoringPage][CalcMemo] Calling calculateScores. Companies: ${includedScoringData.length}, Active Weights: ${Object.keys(activeAndAccessibleWeights).length}, NormMode: ${normalizationMode}, ImputeMode: ${imputationMode}`);
        try {
            ...
            // --- B2Gold specific logging (replace 25 with actual ID or use name) ---
            const B2GOLD_COMPANY_ID_EXAMPLE = 25;
            const b2GoldScoreData = scores.find(s => s.companyId === B2GOLD_COMPANY_ID_EXAMPLE || s.companyName?.toLowerCase().includes("b2gold"));
            if (b2GoldScoreData) {
                console.log(`[ScoringPage][CalcMemo][B2GoldCheck] B2Gold (or similar) IS PRESENT in calculatedScores output. Score: ${b2GoldScoreData.score}, Status: ${b2GoldScoreData.status}`);
            } else if (scores.length > 0) { // Only warn if scores were actually produced for other companies
                 console.warn(`[ScoringPage][CalcMemo][B2GoldCheck] B2Gold (or similar) NOT FOUND in final calculatedScores list (Total scores: ${scores.length}).`);
            } else {
                 console.log(`[ScoringPage][CalcMemo][B2GoldCheck] No scores were produced by calculateScores, so B2Gold (or similar) would not be present.`);
            }
            return Array.isArray(scores) ? scores : EMPTY_SCORE_ARRAY;
        } catch (error: any) {
            console.error('[ScoringPage][CalcMemo] Critical error during calculateScores execution:', error.message, error.stack);
            return EMPTY_SCORE_ARRAY;
        }
    }, [
        ...
        normalizationMode, imputationMode // Added new dependencies for scoring modes
    ]);

    // Memoized filtering of scores by selected status (UI only)
    const filteredScores = useMemo(() => {
        ...
        return calculatedScores.filter(score => {
            // The 'status' field is now directly available on the CompanyScore object
    ...
    // Generates user-friendly messages for empty or loading states
    const getEmptyStateMessage = () => {
        ...
        if (!isScoringDataLoading && scoringCompanyData.length === 0 && filteredCompanyIds && filteredCompanyIds.length > 0) return 'Company details could not be loaded for the filtered set. This might be a temporary issue.';
        ...
        if (Object.values(metricWeights).every(w => w === 0) && metricsForScoring.length > 0) return 'No metrics are weighted. Please adjust metric weights to see scores, or use the "Set Max" button.';
        ...
        if (calculatedScores.length === 0 && metricsForScoring.length > 0 && Object.values(metricWeights).some(w => w > 0) && !overallLoading) {
             return 'Scores could not be calculated for the current set of companies. This might be due to missing critical data for all selected metrics, or invalid data ranges. Try adjusting filters or weights. Check debug logs for more details.';
        }
    ...
    // Handler to open the debug modal for a specific company
    const handleShowDebugModal = (scoreItem: CompanyScore) => {
    ...
    useEffect(() => {
        ...
        if (filteredCompanyIds?.includes(B2GOLD_COMPANY_ID_EXAMPLE)) {
             ...
             console.warn(`[ScoringPage][ContextWatch][B2GoldCheck] B2Gold (ID ${B2GOLD_COMPANY_ID_EXAMPLE}) IS NOT IN current filteredCompanyIds from context.`);
        }
    }, [filteredCompanyIds]);
    ...
    return (
        <PageContainer title="Company Scoring Engine" description={pageDescription} className="relative isolate flex flex-col flex-grow">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-grow min-h-0"> {/* Added flex-grow and min-h-0 here */}
                {/* Metric Weights Card (lg:col-span-1) */}
                <div className="lg:col-span-1 space-y-6 flex flex-col min-h-0"> {/* Added flex flex-col and min-h-0 */}
                    <Card className="bg-navy-800 border border-navy-700 shadow-lg overflow-hidden flex flex-col flex-grow"> {/* Added flex flex-col and flex-grow */}
                            ...
                            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <Select value={normalizationMode} onValueChange={(value) => setNormalizationMode(value as NormalizationMode)} disabled={overallLoading}>
                                        ...
                                        <SelectContent className="bg-navy-700 border-navy-600 text-white text-xs z-[51]"> {/* Ensure z-index for dropdown */}
                        ...
                        <CardContent className="space-y-3 px-4 sm:px-6 pb-4 sm:pb-6 flex-grow overflow-y-auto custom-scrollbar"> {/* Added flex-grow */}
                            {loadingRanges && <div className="text-center text-gray-400 p-3"><LoadingIndicator message="Loading Metric Ranges..." /></div>}
                            {!loadingRanges && Object.keys(groupedMetricsForUI).length === 0 && <p className="text-gray-400 p-3 text-center text-sm">No metric categories available.</p>}
                ...
                {/* Ranked Companies Section (lg:col-span-2) */}
                <div className="lg:col-span-2 space-y-4 flex flex-col min-h-0"> {/* Added flex flex-col and min-h-0 */}
                    <Card className="bg-navy-800 border border-navy-700 shadow-lg flex flex-col flex-grow"> {/* Added flex flex-col and flex-grow */}
                        ...
                        <CardContent className="px-1.5 sm:px-3 md:px-4 pb-4 pt-3 flex-grow overflow-hidden"> {/* Added flex-grow and overflow-hidden */}
                            <ScrollArea className="h-full w-full custom-scrollbar"> {/* Use ScrollArea here for the content */}
            ...
            <Dialog open={isDebugModalOpen} onOpenChange={setIsDebugModalOpen}>
                <DialogContent className="max-w-3xl w-[90vw] md:w-full h-[85vh] bg-navy-850 border-navy-700 text-gray-300 flex flex-col p-0 shadow-2xl rounded-lg">
                    ...
                    <div className="flex-grow overflow-hidden p-3 sm:p-4 pt-0">
                        <ScrollArea className="h-full w-full pr-2 sm:pr-3 custom-scrollbar"> {/* Added custom-scrollbar if you have one */}
                            ...
                            {selectedDebugCompany && Object.entries(selectedDebugCompany.breakdown)
                                ...
                                <div key={metricDbCol} className="mb-1.5 p-1.5 border-t border-navy-700/60 text-[0.6rem] sm:text-xs leading-snug bg-navy-900/30 rounded-sm">
                                    ...
                                    <p className="ml-2">Raw Value: <span className="text-gray-400">{String(compDetails.rawValue)}</span></p>
                                    <p className="ml-2">Processed (Num/Inf): <span className="text-gray-400">{compDetails.processedValue?.toFixed(3) ?? String(compDetails.processedValue)}</span></p>
                                    {compDetails.imputedValue !== undefined && compDetails.imputedValue !== null && <p className="ml-2">Imputed ({compDetails.imputationMethodApplied || imputationMode}): <span className="text-orange-300">{compDetails.imputedValue?.toFixed(3)}</span></p>}
                                     <p className="ml-2">Value for Norm: <span className="text-purple-400">{compDetails.valueUsedForNormalization?.toFixed(3) ?? String(compDetails.valueUsedForNormalization)}</span></p>
