...
export function SubscribePage() {
    ...
    const handleSubscribe = async (priceId: string | null, planName: string) => {
        ...
        try {
            const successUrl = `${window.location.origin}/onboarding?session_id={CHECKOUT_SESSION_ID}`; // Updated to onboarding
        ...
        >
            ...
            <div className="relative z-0 pt-8 pb-12">
                ...
                <div className="text-center mb-10 md:mb-12">
                    ...
                    <Typography variant="body" className="mt-3 max-w-2xl mx-auto text-surface-white/70 text-sm sm:text-base"> Unlock powerful analytics for Canadian miners with MapleAurum. </Typography>
                </div>
                ...
                {isAutoCheckoutLoading && (
                    <div className="text-center py-4 max-w-5xl mx-auto mb-8 px-4">
                        ...
                        <Typography variant="body" className="mt-2 text-surface-white">Redirecting to checkout...</Typography>
                    </div>
                )}
                                    ...
                                    <div className="mt-8">
                                            ...
                                            onClick={() => {
                                                ...
                                                if (clickAuthLoading) {
                                                    ...
                                                    navigate('/auth', {
                                                        ...
                                                        search: `?plan=${plan.name}&interval=${billingInterval}&signup=true`,
                                                    });
                                        ...
                                        {!session && !user && !isFree && (
                                            <Typography variant="caption" className="mt-2 text-center text-gray-400 text-xs">
                                                <Link to="/auth" state={{ from: `/subscribe?plan=${plan.name}&interval=${billingInterval}` }} search={{ signup: true, plan: plan.name, interval: billingInterval }} className="text-cyan-400 hover:underline">Sign up</Link> or <Link to="/auth" state={{ from: `/subscribe?plan=${plan.name}&interval=${billingInterval}` }} className="text-cyan-400 hover:underline">log in</Link> to subscribe
                                            </Typography>
                                        )}
                                    </div>
                                </div>
