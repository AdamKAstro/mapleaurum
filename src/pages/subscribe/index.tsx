...
const STRIPE_MODE_CLIENT = import.meta.env.VITE_STRIPE_MODE || 'live'; 
...
const plansData: PlanDisplayData[] = [
    ...
    ...(STRIPE_MODE_CLIENT === 'test'
        ? [ 
            {
                ...
                // **** THIS IS WHERE YOU PUT YOUR NEW *RECURRING* TEST PRICE ID ****
                // The ID 'price_1RMJ3pAst4LlpL7pXTO1bVli' was a ONE-TIME price.
                // Replace with the new RECURRING price ID, e.g., 'price_1ROT6GAst4LlpL7pRASKFwYA' if that's the correct recurring one.
                priceIdMonthly: import.meta.env.VITE_STRIPE_PREMIUM_MONTHLY_PRICE_ID_TEST || 'price_1ROT6GAst4LlpL7pRASKFwYA', // <<<<< UPDATE THIS FALLBACK
                priceIdYearly: import.meta.env.VITE_STRIPE_PREMIUM_YEARLY_PRICE_ID_TEST || '', 
            ...
            },
            {
                ...
                priceIdMonthly: import.meta.env.VITE_STRIPE_PRO_MONTHLY_PRICE_ID_TEST || 'YOUR_RECURRING_TEST_PRO_MONTHLY_PRICE_ID', // <<<<< UPDATE
                priceIdYearly: import.meta.env.VITE_STRIPE_PRO_YEARLY_PRICE_ID_TEST || 'YOUR_RECURRING_TEST_PRO_YEARLY_PRICE_ID', // <<<<< UPDATE
        ...
        : [ // Live Mode Plans - Ensure these use LIVE RECURRING price IDs
            {
                name: 'Pro',
                priceMonthlyDisplay: '$40', priceYearlyDisplay: '$420', periodMonthly: '/month', periodYearly: '/year',
...
// ... (The rest of your SubscribePage component, including useEffects, handlePaymentForLoggedInUser, admin email form, and render logic,
//      should be the same as the full version I provided in the immediately preceding response.
//      The critical change was the body object for supabase.functions.invoke and the logging.
//      The content below is from that previous robust version.)
...
export function SubscribePage() {
    ...
    useEffect(() => {
        console.log(`[SubscribePage] Using STRIPE_MODE_CLIENT: ${STRIPE_MODE_CLIENT}`);
        console.log(`[SubscribePage] Effective Stripe Publishable Key: ${STRIPE_PUBLISHABLE_KEY_TO_USE ? 'Set (' + STRIPE_PUBLISHABLE_KEY_TO_USE.substring(0,10) + '...)' : 'NOT SET'}`);
        plansData.filter(p => p.tier !== 'free').forEach(plan => {
            ...
            console.log(`[SubscribePage] Plan Check: ${plan.name} (Tier: ${plan.tier}, Yearly: ${isYearly}) - Price ID: '${priceId || 'N/A'}', Buy Button ID: '${buyButtonId || 'N/A'}'`);
            if (!priceId && (session && user && plan.tier !== 'free')) console.warn(`[SubscribePage] Missing Price ID for logged-in user flow: ${plan.name} ${isYearly ? '(Yearly)' : '(Monthly)'}`);
            if (!STRIPE_PUBLISHABLE_KEY_TO_USE) console.warn(`[SubscribePage] VITE_STRIPE_PUBLISHABLE_KEY (for mode ${STRIPE_MODE_CLIENT}) is not set! Stripe buttons for guests might not work.`);
            if (!(session && user) && !buyButtonId && plan.tier !== 'free') console.warn(`[SubscribePage] Stripe Buy Button ID for ${plan.name} (${isYearly ? 'Yearly' : 'Monthly'}) is missing! Button may not render for guests.`);
        });
    ...
    const handlePaymentForLoggedInUser = async (plan: PlanDisplayData) => {
        if (!user || !session) {
            ...
            navigate('/login?action=signup&from=subscribe&plan=' + plan.tier, { state: { from: { pathname: '/subscribe' } } });
            return;
        }
        ...
        if (!priceIdToUse) {
            console.error(`[SubscribePage] CRITICAL: Stripe Price ID is missing for ${plan.name} (${isYearly ? 'Yearly' : 'Monthly'}). Please check plansData configuration for STRIPE_MODE_CLIENT: ${STRIPE_MODE_CLIENT}.`);
            alert("Configuration error: This plan's Price ID is missing. Please contact support.");
            return;
        }
        ...
        const checkoutBody = {
            ...
            mode: 'subscription' as 'subscription', // Explicitly set mode
            plan_name: plan.name,
            interval: isYearly ? 'year' : 'month' as 'month' | 'year',
        };
        
        console.log(`[SubscribePage] Logged-in user: Invoking 'stripe-checkout' for user ${user.id}, plan ${plan.name} (tier ${plan.tier}), price ID: ${priceIdToUse}. Body being sent:`, JSON.stringify(checkoutBody, null, 2));

        try {
            const { data, error: functionError } = await supabase.functions.invoke('stripe-checkout', {
                body: checkoutBody, // Pass the object directly, supabase-js client will stringify
            });

            console.log('[SubscribePage] Raw response (logged-in user) from stripe-checkout:', { data, functionError });

            if (functionError) {
                let displayError = 'Checkout initialization failed.';
                const errorContext = (functionError as any).context; // Supabase FunctionsHttpError often has context
                if (errorContext?.error?.message) displayError += ` Server responded: ${errorContext.error.message}`;
                else if (errorContext?.details) displayError += ` Server responded: ${errorContext.details}`;
            ...
            if (data && data.url) {
                ...
                // setIsProcessing(null) will not be reached if redirection happens
            } else {
                console.error('[SubscribePage] Failed to get Stripe Checkout session URL from function. Response data:', data);
                alert('Could not retrieve a valid checkout session. Please check the console and contact support.');
                setIsProcessing(null);
            }
        } catch (error: any) { 
            console.error('[SubscribePage] Checkout process error (outer catch for invoke issues):', error.message, error);
            alert(`An unexpected error occurred: ${error.message}. Please try again or contact support at support@mapleaurum.com.`);
            setIsProcessing(null);
        }
    };
    ...
    const handleSendTestEmail = async (e: FormEvent) => { 
        e.preventDefault();
        if (!isAdmin) { setEmailStatus('Unauthorized.'); return; }
        if (!testEmail) { setEmailStatus('Recipient email needed.'); return; }
        ...
        try {
            ...
            if (sessionErr || !currentAuthSession) throw new Error(sessionErr?.message || 'No active session for auth token.');
            ...
            setEmailStatus('Test email sent successfully!'); setTestEmail('');
        } catch (error: any) { setEmailStatus(`Error: ${error.message}`);
        ...
        >
            ...
            {!STRIPE_PUBLISHABLE_KEY_TO_USE && 
                <div className="bg-red-600 text-white p-3 text-center text-sm font-medium fixed top-0 left-0 right-0 z-[100]">
                    Warning: Stripe Publishable Key for {STRIPE_MODE_CLIENT.toUpperCase()} mode is NOT configured. Payment buttons for guests may not work.
                </div>
            }
                ...
                {isAdmin && ( 
                    <div className="max-w-md mx-auto mb-8 p-6 bg-navy-700/60 border border-navy-600/50 rounded-lg backdrop-blur-sm">
                        ...
                        <form onSubmit={handleSendTestEmail} className="space-y-4">
                            <div><Label htmlFor="test-email" className="text-gray-300">Recipient Email</Label><Input id="test-email" type="email" value={testEmail} onChange={(e) => setTestEmail(e.target.value)} placeholder="e.g., recipient@example.com" required disabled={isEmailSending} className="bg-navy-600/80 border-navy-500 text-white" /></div>
                            <div><Label htmlFor="test-subject" className="text-gray-300">Subject</Label><Input id="test-subject" type="text" value={testSubject} onChange={(e) => setTestSubject(e.target.value)} placeholder="Test Email Subject" required disabled={isEmailSending} className="bg-navy-600/80 border-navy-500 text-white" /></div>
                            <div><Label htmlFor="test-message" className="text-gray-300">Message</Label><Input id="test-message" type="text" value={testMessage} onChange={(e) => setTestMessage(e.target.value)} placeholder="Test email body." required disabled={isEmailSending} className="bg-navy-600/80 border-navy-500 text-white" /></div>
                ...
                <div className="grid grid-cols-1 gap-6 sm:gap-8 lg:grid-cols-3 max-w-5xl mx-auto">
                    {plansData.map((plan) => {
                        ...
                        if (plan.tier === 'free') {
                            ...
                            else currentActionHandler = null;
                        } else { 
                            if (session && user) { 
                                ...
                                if (!showStripeBuyButtonElement) { // If Stripe button CANNOT be shown (e.g. missing ID)
                                    currentActionHandler = () => { // Custom button will redirect to login/signup
                                        setIsRedirecting(true);
                                        navigate(`/login?action=signup&plan=${plan.tier}&yearly=${isYearly}`, { state: { from: { pathname: '/subscribe' } }});
                                    };
                                }
                                // If Stripe button *can* be shown, currentActionHandler remains null for the custom button
                                // The disabled state for the custom button if Stripe button is meant to show:
                                if (showStripeBuyButtonElement) {
                                     isButtonDisabled = true; // Disable custom button if Stripe button will show
                                } else {
                                    if (!isButtonCurrentlyProcessing) isButtonDisabled = isLoadingOverall || isRedirecting;
                                }
                            }
                        }
                        if (isButtonCurrentlyProcessing && plan.tier !== 'free') { // Don't show "Processing..." for the static free button
                                ...
                                <div className={cn('relative flex flex-col h-full rounded-xl border p-6 w-full', plan.popular ? 'bg-navy-700/50 border-cyan-700/50' : 'bg-navy-800/60 border-navy-700/50', 'backdrop-blur-sm')}>
                                    ...
                                    <div className="mt-8">
                                        {showStripeBuyButtonElement ? (
                                            <stripe-buy-button
                                                buy-button-id={currentBuyButtonIdForStripeElement!} // Assert non-null as showStripeBuyButtonElement checks it
                                                publishable-key={STRIPE_PUBLISHABLE_KEY_TO_USE!} // Assert non-null
                                                // client-reference-id and customer-email are useful if your Stripe webhook handles them
                                                // For new customers, Stripe collects email on its page.
                                            >
                                            </stripe-buy-button>
                                        ) : (
                                            <Button
                                                ...
                                                style={plan.tier === 'free' && session && actualUserTier !== 'free' ? { pointerEvents: 'none' } : {}}
                                            >
