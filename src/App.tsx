...
// import TestIcons from './components/TestIcons'; // Assuming for dev only
...
import { LoginPage } from './pages/login'; // LoginPage will handle login/signup UI
import { ForgotPasswordPage } from './pages/forgot-password';
import { ResetPasswordPage } from './pages/reset-password';
// AuthPage is now effectively LoginPage
import { OnboardingPage } from './pages/onboarding';
import { CompaniesPage } from './pages/companies';
import { SubscribePage } from './pages/subscribe';
// import { SuccessPage } from './pages/success'; // Potentially deprecated
...
function App() {
    ...
    // const showDebugSelector = import.meta.env.DEV && typeof DebugTierSelector !== 'undefined';
    const showDebugSelector = false; // Keep disabled for now unless explicitly re-enabled
            ...
            <ThemeProvider>
                <AuthProvider>
                    <SubscriptionProvider>
                        <CurrencyProvider>
                            <FilterProvider>
                                <ErrorBoundary fallback={<div className="flex items-center justify-center h-screen text-red-500 bg-navy-900">Something went terribly wrong! Please reload the page.</div>}>
                                    <Helmet>
                                            ...
                                            <main className="flex-1 overflow-y-auto relative isolate">
                                                ...
                                                <ErrorBoundary fallback={<div className="p-6 text-red-400">Error loading this page section.</div>}>
                                                    <Routes>
                                                        ...
                                                        <Route path="/auth" element={<Navigate to="/login" replace />} /> {/* Redirect /auth to /login */}
                                                        <Route path="/forgot-password" element={<ForgotPasswordPage />} />
                                                        <Route path="/reset-password" element={<ResetPasswordPage />} />
                                                        <Route path="/subscribe" element={<SubscribePage />} />
                                                        {/* <Route path="/subscribe/success" element={<SuccessPage />} />  Consider removing */}
